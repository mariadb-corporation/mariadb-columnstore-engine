...
create table qa_nation (n_id int, n_name char(20), regionid int) engine=infinidb;
create table qa_customer (c_id int, c_custname char(20), nationid int) engine=infinidb;
create table qa_region (r_id int, r_name char(20)) engine=infinidb;
create table qa_orders (o_id int, o_name char(20), custid int) engine=infinidb;
...
from qa_region r 
	...
	join qa_customer c on n.n_id = c.nationid 
	join qa_orders o on c.c_id = o.custid
where c.c_id in (select c1.c_id from qa_customer c1) order by r.r_name, n.n_name;
