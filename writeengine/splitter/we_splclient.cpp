...
void WESplClient::sendAndRecv()
{
  ...
  if (this->fCpiFailed)
  {
    ...
    if (fOwner.getDebugLvl())
      cout << aRedCol << "Bulk load FAILED on PM " << getPmId() << aDefCon << endl;
  }
  else if (this->fCpiPassed)
  {
    ...
    if (fOwner.getDebugLvl())
      cout << aGreenCol << "Bulk load Finished Successfully on PM " << getPmId() << aDefCon << endl;
  }
...
void WESplClient::send()
{
  if ((!fSendQueue.empty()) && (getDataRqstCount() > 0))
  {
    ...
    std::scoped_lock aLock(fSentQMutex);
    ...
    if (aLen > 0)
    {
      std::scoped_lock aLock(fWriteMutex);
...
void WESplClient::printStats()
{
  if (fOwner.getDebugLvl())
  {
    ...
    cout << "\tInserted/Read Rows   " << fRowsUploadInfo.fRowsInserted << "/" << fRowsUploadInfo.fRowsRead
         << endl;
    ...
    if (!fBadDataFile.empty())
      cout << "\tBad Data Filename    " << fBadDataFile << endl;

    if (!fErrInfoFile.empty())
      cout << "\tError Filename       " << fErrInfoFile << endl;
...
void WESplClient::add2ColOutOfRangeInfo(int ColNum, execplan::CalpontSystemCatalog::ColDataType ColType,
