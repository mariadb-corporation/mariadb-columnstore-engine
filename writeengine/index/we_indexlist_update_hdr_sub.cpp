...
const int IndexList::updateHdrSub(const RID& newRid, const uint64_t& key)
{
  ...
  if (m_curIdxRidListHdr.firstIdxRidListEntry.type == (int)LIST_NOT_USED_TYPE)
  {
  ...
  switch (m_nextType)
  {
    ...
    case LIST_RID_TYPE:  // Header is full, need a new sub-block
      ...
      m_curIdxRidListHdr.nextIdxRidListPtr.llp = ((IdxRidListHdrPtr*)&newIdxListEntry)->llp;
    ...
    default: rc = ERR_IDX_LIST_INVALID_UP_HDR; break;
  }  // end of switch
 ...
 *    failure    - it did not create the index list header
 ***********************************************************/
const int IndexList::addRidInSub(const RID& newRid, IdxRidListPtr& lastIdxRidListPtr)
{
  int rc = NO_ERROR;
  int maxCount;
  int count;
 ...
 *    fail
 ***********************************************************/
const int IndexList::moveRidsToNewSub(FILE* pFile, const RID& oldRid, const RID& newRid,
                                      IdxEmptyListEntry* newIdxListEntryPtr)
{
  int rc;
