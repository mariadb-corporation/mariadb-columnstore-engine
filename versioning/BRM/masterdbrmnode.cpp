...
void MasterDBRMNode::msgProcessor()
{
    ...
    while (!die)
    {
        ...
        switch (cmd)
        {
            ...
            case SETREADONLY:
                doSetReadOnly(p->sock, true);
                continue;

            case SETREADWRITE:
                doSetReadOnly(p->sock, false);
                continue;

            case GETREADONLY:
                doGetReadOnly(p->sock);
                continue;

            case GET_SYSTEM_CATALOG:
                doGetSystemCatalog(msg, p);
                continue;
        }
        ...
        switch (cmd)
        {
            ...
            case GET_UNIQUE_UINT32:
                doGetUniqueUint32(msg, p);
                continue;

            case GET_UNIQUE_UINT64:
                doGetUniqueUint64(msg, p);
                continue;

            case GET_SYSTEM_STATE:
                doGetSystemState(msg, p);
                continue;

            case SET_SYSTEM_STATE:
                doSetSystemState(msg, p);
                continue;

            case CLEAR_SYSTEM_STATE:
                doClearSystemState(msg, p);
                continue;

            case SM_RESET:
                doSessionManagerReset(msg, p);
                continue;

            case GET_UNCOMMITTED_LBIDS:
                doGetUncommittedLbids(msg, p);
                continue;
        }
        ...
        {
            case GET_TABLE_LOCK:
                doGetTableLock(msg, p);
                continue;

            case RELEASE_TABLE_LOCK:
                doReleaseTableLock(msg, p);
                continue;

            case CHANGE_TABLE_LOCK_STATE:
                doChangeTableLockState(msg, p);
                continue;

            case CHANGE_TABLE_LOCK_OWNER:
                doChangeTableLockOwner(msg, p);
                continue;

            case GET_ALL_TABLE_LOCKS:
                doGetAllTableLocks(msg, p);
                continue;

            case RELEASE_ALL_TABLE_LOCKS:
                doReleaseAllTableLocks(msg, p);
                continue;

            case GET_TABLE_LOCK_INFO:
                doGetTableLockInfo(msg, p);
                continue;
        ...
        switch (cmd)
        {
            ...
            case GETDBROOTOFVBOID:
                doGetDBRootOfVBOID(msg, p);
                continue;

            case GETVBOIDTODBROOTMAP:
                doGetVBOIDToDBRootMap(msg, p);
                continue;
        }
        ...
        switch (cmd)
        {
            case START_AI_SEQUENCE:
                doStartAISequence(msg, p);
                continue;
            ...
            case RESET_AI_SEQUENCE:
                doResetAISequence(msg, p);
                continue;
            ...
            case RELEASE_AI_LOCK:
                doReleaseAILock(msg, p);
                continue;
