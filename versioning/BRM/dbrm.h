...
class DBRM
{
 public:
   ...
   */
  EXPORT int lookupLocal(LBID_t lbid, VER_t verid, bool vbFlag, OID_t& oid, uint16_t& dbRoot,
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int lookupLocal(OID_t oid, uint32_t partitionNum, uint16_t segmentNum, uint32_t fileBlockOffset,
                         LBID_t& lbid) throw();

  /** @brief A dbroot-specific version of lookupLocal() */
  EXPORT int lookupLocal_DBroot(OID_t oid, uint32_t dbroot, uint32_t partitionNum, uint16_t segmentNum,
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int lookupLocalStartLbid(OID_t oid, uint32_t partitionNum, uint16_t segmentNum,
                                  uint32_t fileBlockOffset, LBID_t& lbid) throw();
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int vssLookup(LBID_t lbid, const QueryContext& qc, VER_t txnID, VER_t* outVer, bool* vbFlag,
   ...
   * @param out (out) The values equivalent to the out parameters in vssLookup() including the individual
   * return codes, ordered as the lbid list.
  ...
  EXPORT VER_t getHighestVerInVB(LBID_t lbid,
                                 VER_t max = std::numeric_limits<VER_t>::max()) const;  // returns
  ...
  EXPORT int createColumnExtent_DBroot(OID_t oid, uint32_t colWidth, uint16_t dbRoot, uint32_t& partitionNum,
                                       uint16_t& segmentNum,
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getLastHWM_DBroot(int OID, uint16_t dbRoot, uint32_t& partitionNum, uint16_t& segmentNum,
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getLocalHWM(OID_t oid, uint32_t partitionNum, uint16_t segmentNum, HWM_t& hwm,
                         int& status) throw();
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int setLocalHWM(OID_t oid, uint32_t partitionNum, uint16_t segmentNum, HWM_t hwm) DBRM_THROW;
  ...
  EXPORT int bulkSetHWMAndCP(const std::vector<BulkSetHWMArg>& hwmArgs,
                             ...
                             const std::vector<CPInfoMerge>& mergeCPDataArgs, VER_t transID) DBRM_THROW;
   ...
   */
  EXPORT int getDbRootHWMInfo(OID_t oid, uint16_t pmNumber, EmDbRootHWMInfo_v& emDbRootHwmInfos) throw();
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getExtentState(OID_t oid, uint32_t partitionNum, uint16_t segmentNum, bool& bFound,
                            int& status) throw();
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getExtents(int OID, std::vector<struct EMEntry>& entries, bool sorted = true,
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getExtentCount_dbroot(int OID, uint16_t dbroot, bool incOutOfService,
                                   uint64_t& numExtents) throw();
   ...
   */
  EXPORT int deletePartition(const std::vector<OID_t>& oids, const std::set<LogicalPartition>& partitionNums,
                             std::string& emsg) DBRM_THROW;
   ...
   */
  EXPORT int restorePartition(const std::vector<OID_t>& oids, const std::set<LogicalPartition>& partitionNums,
                              std::string& emsg) DBRM_THROW;
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int getUncommittedLBIDs(VER_t transID, std::vector<LBID_t>& lbidList) throw();
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int vbRollback(VER_t transID, const LBIDRange_v& lbidList) DBRM_THROW;
   ...
   * @return 0 on success, non-0 on error (see brmtypes.h)
   */
  EXPORT int vbRollback(VER_t transID, const std::vector<LBID_t>& lbidList) DBRM_THROW;
  ...
  EXPORT uint64_t getTableLock(const std::vector<uint32_t>& pmList, uint32_t tableOID, std::string* ownerName,
                               uint32_t* ownerPID, int32_t* ownerSessionID, int32_t* ownerTxnID,
                               LockState state);
