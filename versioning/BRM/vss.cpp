...
std::vector<std::mutex> VSS::vssMutexes_(VssFactor);
...
// assumes write lock is held
// void VSS::growForLoad(uint32_t elementCount)
// {
//   int allocSize;
//   key_t newshmkey;
//   char* newshmseg;
...
void VSS::getUncommittedLBIDs(VER_t txnID, vector<LBID_t>& lbids)
{
  ...
  for (i = 0; i < vss->capacity; i++)
    if (storage[i].lbid != -1 && storage[i].verID == txnID)
    {
      ...
      {
        log("VSS::getUncommittedLBIDs(): found an unlocked block with that TxnID", logging::LOG_TYPE_DEBUG);
        throw logic_error("VSS::getUncommittedLBIDs(): found an unlocked block with that TxnID");
      }

      if (storage[i].vbFlag == true)
      {
        log("VSS::getUncommittedLBIDs(): found a block with that TxnID in the VB", logging::LOG_TYPE_DEBUG);
