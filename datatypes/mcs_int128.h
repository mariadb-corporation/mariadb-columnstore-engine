/*
   ...
   MA 02110-1301, USA. 
*/ 
#ifndef MCS_TSINT128_H_INCLUDED
#define MCS_TSINT128_H_INCLUDED
...
class TSInt128
{
  public:
    ...
    // unaligned argument
    TSInt128(const int128_t* x) { assignInt128PtrPtr(&s128Value, x); }

    // unaligned argument
    TSInt128(const unsigned char* x) { assignInt128PtrPtr(&s128Value, x); }

    TSInt128(const TSInt128& x) { s128Value = x.s128Value; }
    ...
    static inline void assignInt128PtrPtr(D* dst, const S* src)
    {
         MACRO_PTR_PTR_128(dst, "=m", src, "m", "xmm0")
    }

    template <typename D>
    static inline void store(D* dst, const int128_t& src)
    {
    ...
    inline void operator=(const TSInt128& x)
    {
      s128Value = x.s128Value;
    }
