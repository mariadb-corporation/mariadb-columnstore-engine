...
MTR = {
    ...
    'mtr-columnstore-multinode-test': [" --suite=columnstore/setup "
                            "--skip-test=rocksdb_hotbackup* --big-test "
                            "--max-save-core=1 --max-save-datadir=1 --verbose-restart --max-test-fail=0 ",

                            " --suite=columnstore/basic,columnstore/bugfixes,columnstore/devregression,"
                            "columnstore/autopilot,columnstore/extended,"
                            "columnstore/multinode,columnstore/oracle "
                            "--skip-test=rocksdb_hotbackup* --big-test "
                            "--max-save-core=1 --max-save-datadir=1 --verbose-restart --max-test-fail=0 "],

    'mtr-columnstore-multinode-setup-test': [" --suite=columnstore/setup "
                            "--skip-test=rocksdb_hotbackup* --big-test "
                            "--max-save-core=1 --max-save-datadir=1 --verbose-restart --max-test-fail=0 "],

    'mtr-columnstore-multinode-maxscale-test': [" --suite=columnstore/basic,columnstore/bugfixes,"
                                                "columnstore/devregression,"
                                                "columnstore/autopilot,columnstore/extended,"
                                                "columnstore/multinode,columnstore/oracle "
                                                "--skip-test=rocksdb_hotbackup* --big-test "
                                                "--max-save-core=1 --max-save-datadir=1 --verbose-restart "
...
ORACLE_PACKAGES = {
    ...
    platform_names.AMD64_RPM_ALIAS: [
...
def installMariaDBTest(product, target):
    mdbci.runMdbci('setup_repo', '--product=' + products_names.PRODS[product],
                   '--product-version=' + target, '--include-unsupported', machine.fullName)
...
parser.add_argument("--architecture", help="Processor architecture (x86_64 / aarch64 / ppc64le)", default="x86_64")
...
if arguments.architecture == platform_names.AMD64_ARCH:
    architecture = platform_names.AMD64_RPM_ALIAS
else:
...
if arguments.use_packages:
    if arguments.install_packages:
        if installMariaDB(arguments.product, arguments.target) != 0:
            exit(1)
    if installMariaDBTest(arguments.product, arguments.target) != 0:
        exit(1)
...
if machine.platform not in ['windows']:
    ...
    if arguments.mtr_param in ['mtr-spider-sqlite-test']:
        ...
        archCfg = ''
        if architecture in [platform_names.AARCH64_ARCH]:
    ...
    if arguments.mtr_param in ['mtr-columnstore-test',
                               'mtr-columnstore-multinode-test',
                               'mtr-columnstore-multinode-setup-test',
                               'mtr-columnstore-multinode-maxscale-test']:
        mdbci.runMdbci('install_product', '--product=mdbe_build', '--product-version=latest', machine.fullName)
        # Hack to get Columstore MTR files
        if runCommand('[ -d {}/plugin/columnstore/columnstore/1pmonly ]'.format(pathToMTR)) != 0:
            runCommand(f"git clone https://github.com/mariadb-corporation/mariadb-columnstore-engine.git;"
                       f"cd mariadb-columnstore-engine;"
                       f'git checkout stable-23.10;'
                       f"sudo cp -r mysql-test/columnstore/* {pathToMTR}/plugin/columnstore/columnstore/")
        ...
        runCommand("sudo mkdir -p {pathToMTR}/suite/columnstore/std_data")
        runCommand(f"sudo cp -r {pathToMTR}/plugin/columnstore/columnstore/std_data/*  "
                   f"{pathToMTR}/suite/columnstore/std_data/")
        ...
        if arguments.mtr_param not in  ['mtr-columnstore-multinode-maxscale-test']:
            sockL = []
            ...
            socketPath = f"--extern port=4006 --extern host=127.0.0.1 " \
                         f"--extern user=testUser --extern password=\\\"{MAX_PASS}\\\""
        host.printInfo(f'Socket path {socketPath}')
        testUser = 'root'
        runCommand(f"sudo sed -i \"/\/tmp\/mysqld.sock/d\" {pathToMTR}/mysql-test-run.pl")
