...
#define NETSNMP_CONFIGURE_OPTIONS " '--prefix=/usr/local/mariadb/columnstore' '--with-mib-modules=disman/event-mib' '--with-cc=gcc' '--with-logfile=/var/log/snmpd.log' '--with-sys-location=Unknown' '--with-sys-contact=root@localhost.localdomain' '--with-default-snmp-version=3' '--with-persistent-directory=/var/net-snmp' '--without-openssl' '--with-ldflags=-Wl,-rpath -Wl,/usr/local/mariadb/columnstore/lib'"

/* default location to look for mibs to load using the above tokens and/or
   those in the MIBS envrionment variable */
#define NETSNMP_DEFAULT_MIBDIRS "$HOME/.snmp/mibs:/usr/local/mariadb/columnstore/share/snmp/mibs"
...
#define NETSNMP_SYS_CONTACT "root@localhost.localdomain"
