...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-MENT-1667') = {
  ...
  mtr:: {
    ...
    commands: [
      ...
      if (pkg_format == 'deb') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "echo "character_set_server=latin1" >> /etc/mysql/mariadb.conf.d/columnstore.cnf"' else 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "echo "" >> /etc/my.cnf.d/columnstore.cnf;echo "character_set_server=latin1" >> /etc/my.cnf.d/columnstore.cnf"',
