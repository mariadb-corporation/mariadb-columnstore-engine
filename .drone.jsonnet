...
local rpm_build_deps = "git cmake make gcc gcc-c++ libaio-devel openssl-devel boost-devel bison snappy-devel flex libcurl-devel libxml2-devel ncurses-devel automake libtool policycoreutils-devel rpm-build lsof iproute pam-devel perl-DBI cracklib-devel expect readline-devel net-tools sysvinit-tools";
...
local platformMap(branch, platform) =
  ...
  "centos:7": "yum install -y " + rpm_build_deps + " && cmake " + cmakeflags + branch_cmakeflags_map[branch] + " -DRPM=centos7 && make -j$(nproc) package",
  "centos:8": "sed -i 's/enabled=0/enabled=1/' /etc/yum.repos.d/CentOS-PowerTools.repo && yum install -y " + rpm_build_deps + " && cmake " + cmakeflags + branch_cmakeflags_map[branch] + " -DRPM=centos8 && make -j$(nproc) package",
