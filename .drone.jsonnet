...
local Pipeline(branch, platform, event, arch='amd64', server='10.9') = {
  ...
  smoke:: {
    ...
    commands: [
      'docker run --volume /sys/fs/cgroup:/sys/fs/cgroup:ro --env DEBIAN_FRONTEND=noninteractive --init --env MCS_USE_S3_STORAGE=0 --cap-add=SYS_PTRACE --name smoke$${DRONE_BUILD_NUMBER} --ulimit core=-1 --privileged --detach ' + img + ' ' + init + ' --unit=basic.target',
  ...
  mtr:: {
    ...
    volumes: [pipeline._volumes.docker],
    commands: [
      'docker run --volume /sys/fs/cgroup:/sys/fs/cgroup:ro --init --env MYSQL_TEST_DIR=' + mtr_path + ' --env DEBIAN_FRONTEND=noninteractive --env MCS_USE_S3_STORAGE=0 --name mtr$${DRONE_BUILD_NUMBER} --ulimit core=-1 --privileged --detach ' + img + ' ' + init + ' --unit=basic.target',
      ...
      'docker exec -t mtr$${DRONE_BUILD_NUMBER} sysctl -w kernel.core_pattern="/tmp/%e_core_dump.%p"',
  ...
  regression:: {
    ...
    commands: [
      ...
      'cd ..',
      'docker run --shm-size=500m --volume /sys/fs/cgroup:/sys/fs/cgroup:ro --init --env DEBIAN_FRONTEND=noninteractive --env MCS_USE_S3_STORAGE=0 --ulimit core=-1 --name regression$${DRONE_BUILD_NUMBER} --privileged --detach ' + img + ' ' + init + ' --unit=basic.target',
      ...
      'docker exec -t regression$${DRONE_BUILD_NUMBER} sysctl -w kernel.core_pattern="/tmp/%e_core_dump.%p"',
  ...
  regressionlog:: {
    ...
    commands: [
      ...
      'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "/core_dump_check.sh mtr /' + result + '/"',
