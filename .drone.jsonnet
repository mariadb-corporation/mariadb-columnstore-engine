...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  smoke:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "yum install -y wget graphviz jeprof procps-ng"',
      if (pkg_format == 'deb') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} sed -i "s/exit 101/exit 0/g" /usr/sbin/policy-rc.d',
      if (pkg_format == 'deb') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "apt update --yes && apt install -y procps graphviz jeprof wget"',
  ...
  mtr:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "yum install -y wget graphviz jeprof procps-ng"',
      if (pkg_format == 'deb') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} sed -i "s/exit 101/exit 0/g" /usr/sbin/policy-rc.d',
      if (pkg_format == 'deb') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "apt update --yes && apt install -y procps graphviz jeprof wget"',
  ...
  mtrlog:: {
    ...
    commands: [
      ...
      'echo "---------- end columnstore debug log ----------"',
      'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "/jemalloc_reports.sh var/run/columnstore/heapprof Mtr"',
      'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "/core_dump_check.sh core /core/ Mtr"',
      'docker cp mtr$${DRONE_BUILD_NUMBER}:/var/run/columnstore/heapprof/ /drone/src/' + result + '/',
  ...
  regression:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'centos' || std.split(platform, ':')[0] == 'rockylinux') then 'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "yum install -y procps-ng graphviz jeprof wget"',
      if (pkg_format == 'deb') then 'docker exec -t regression$${DRONE_BUILD_NUMBER} sed -i "s/exit 101/exit 0/g" /usr/sbin/policy-rc.d',
      if (pkg_format == 'deb') then 'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "apt update --yes && apt install -y procps graphviz jeprof wget"',
  ...
  smokelog:: {
    ...
    commands: [
      ...
      'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "/jemalloc_reports.sh var/run/columnstore/heapprof Smoke"',
      'docker cp smoke$${DRONE_BUILD_NUMBER}:/core/ /drone/src/' + result + '/',
      'docker cp smoke$${DRONE_BUILD_NUMBER}:/var/run/columnstore/heapprof/ /drone/src/' + result + '/',
  ...
  regressionlog:: {
    ...
    commands: [
      ...
      'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "/jemalloc_reports.sh var/run/columnstore/heapprof Regression"',
      'docker cp regression$${DRONE_BUILD_NUMBER}:/core/ /drone/src/' + result + '/',
      'docker cp regression$${DRONE_BUILD_NUMBER}:/var/run/columnstore/heapprof/ /drone/src/' + result + '/',
      'ls -l /drone/src/' + result,
