...
local Pipeline(branch, platform, event, arch="amd64", server="10.6-enterprise", customBootstrapParamsKey="", customBuildEnvCommandsMapKey="", ignoreFailureStepList=[]) = {
  ...
  custom_tests:: {
    ...
    volumes: [pipeline._volumes.docker, pipeline._volumes.mdb],
    commands: [
      prepareTestContainer(getContainerName("custom_tests"), result, true, true, true),

      "apk add bash &&" + get_build_command("../tests/custom/cpimport-rollback.sh")
    ],
    [if (std.member(ignoreFailureStepList, "custom_tests")) then "failure"]: "ignore",
  },
