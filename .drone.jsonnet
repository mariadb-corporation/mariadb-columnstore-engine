...
local cmakeflags = '-DCMAKE_BUILD_TYPE=Debug -DPLUGIN_COLUMNSTORE=YES -DPLUGIN_XPAND=NO -DPLUGIN_MROONGA=NO -DPLUGIN_ROCKSDB=NO ' +
                   ...
                   '-DBUILD_CONFIG=mysql_release -DMYSQL_MAINTAINER_MODE=OFF';
...
local platformMap(platform) =
  ...
  local platform_map = {
    'opensuse/leap:15': 'zypper ' + rpm_build_deps + ' pcre2-devel cmake libboost_system-devel libboost_filesystem-devel libboost_thread-devel libboost_regex-devel libboost_date_time-devel libboost_chrono-devel libboost_atomic-devel gcc-fortran && cmake ' + cmakeflags + ' -DRPM=sles15 && make -j$(nproc) package',
    'centos:7': 'yum install -y epel-release && yum install -y cmake3 && ln -s /usr/bin/cmake3 /usr/bin/cmake && yum ' + rpm_build_deps + ' pcre2-devel && cmake ' + cmakeflags + ' -DRPM=centos7 && make -j$(nproc) package',
    'centos:8': "dnf install -y libgcc libarchive && sed -i 's/enabled=0/enabled=1/' /etc/yum.repos.d/*PowerTools.repo && yum " + rpm_build_deps + ' cmake && cmake ' + cmakeflags + ' -DRPM=centos8 && make -j$(nproc) package',
...
local Pipeline(branch, platform, event, arch='amd64') = {
  ...
  local img = if (platform == 'centos:7') then platform else 'romcheck/' + std.strReplace(platform, '/', '-'),
  ...
  smoke:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'debian' || std.split(platform, ':')[0] == 'ubuntu') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "apt update -y && apt install -y rsyslog hostname && apt install -y -f /result/*.' + pkg_format + '"' else '',
  ...
  mtr:: {
    ...
    commands: [
      ...
      if (pkg_format == 'deb') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "apt update -y && apt install -y rsyslog hostname patch && apt install -y -f /result/*.' + pkg_format + '"' else '',
  ...
  regression:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'debian' || std.split(platform, ':')[0] == 'ubuntu') then 'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "apt update -y && apt install -y tar liblz4-tool wget rsyslog hostname && apt install -y -f /result/*.' + pkg_format + '"' else '',
