...
local Pipeline(branch, platform, event, arch="amd64", server="10.6-enterprise", customBootstrapParamsKey="", customBuildEnvCommandsMapKey="", ignoreFailureStepList=[]) = {
  ...
  local regression_tests_base = if (event == "cron" || event == "custom") then [
    "test000.sh",
    "test001.sh",
    "test005.sh",
    "test006.sh",
  ...
  steps: [
         ...
         (if (event == "custom") then []
          else if (platform == "rockylinux:8" && arch == "amd64" && customBootstrapParamsKey == "gcc-toolset") then [pipeline.dockerfile] + [pipeline.dockerhub] + [pipeline.multi_node_mtr] + [pipeline.multinode_mtrlog] + [pipeline.publish("multinode-mtrlog")]
          else [pipeline.mtr] + [pipeline.mtrlog] + [pipeline.publish("mtrlog")]) +
         [pipeline.regression(regression_tests[i], if (i == 0) then (if (event == "custom") then ["publish pkg", "publish cmapi build"] else ["mtr", "publish pkg", "publish cmapi build"]) else [regression_tests[i - 1]]) for i in indexes(regression_tests)] +
