...
local Pipeline(branch, platform, event, arch='amd64', server='10.9') = {
  ...
  mtrlog:: {
    ...
    commands: [
      ...
      'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "/core_dump_check.sh core /' + result + '/" Mtr',
      'echo "---------- end columnstore debug log ----------"',
  ...
  regressionlog:: {
    ...
    commands: [
      ...
      'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "/core_dump_check.sh core /' + result + '/" Regression',
