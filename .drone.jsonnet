...
local Pipeline(branch, platform, event, arch="amd64", server="10.6-enterprise", customBootstrapParamsKey="", customBuildEnvCommandsMapKey="", ignoreFailureStepList=[]) = {
  ...
  steps: [
         ...
         // MTR commented out to save time
         ...
         [pipeline.regression(regression_tests[i], if (i == 0) then ["smoke", "publish pkg", "publish cmapi build"] else [regression_tests[i - 1]]) for i in indexes(regression_tests)] +
...
local AllPipelines =
  ...
  // clang, libcpp, extra_servers, UBSan, ASan — all disabled for this test run
  [];
