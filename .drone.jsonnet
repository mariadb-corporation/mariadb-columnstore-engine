...
local Pipeline(branch, platform, event, arch='amd64', server='10.6-enterprise') = {
  ...
  smoke:: {
    ...
    commands: [
      ...
      if (pkg_format == 'rpm') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "yum install -y wget gdb gawk epel-release which rsyslog hostname procps-ng"' else 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "apt clean -y && apt update --yes && apt install -y gdb gawk rsyslog hostname procps wget"',
  ...
  mtr:: {
    ...
    commands: [
      ...
      if (pkg_format == 'deb') then 'docker exec -t mtr$${DRONE_BUILD_NUMBER} bash -c "apt clean -y && apt update --yes && apt install -y procps wget tar liblz4-tool"',
  ...
  prepare_regression:: {
    ...
    commands: [
      ...
      if (pkg_format == 'rpm') then 'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "yum install -y wget gawk gdb gcc-c++ epel-release diffutils tar findutils lz4 wget which rsyslog hostname procps-ng elfutils"' else 'docker exec -t regression$${DRONE_BUILD_NUMBER} bash -c "apt clean -y && apt update --yes && apt install -y wget tar liblz4-tool procps wget findutils gawk gdb rsyslog hostname g++ elfutils"',
