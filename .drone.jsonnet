...
local platforms = {
  develop: ['centos:7', 'rockylinux:8', 'rockylinux:9', 'debian:11', 'debian:bookworm', 'ubuntu:20.04', 'ubuntu:22.04'],
  'develop-6': ['centos:7', 'rockylinux:8', 'debian:11', 'debian:bookworm', 'ubuntu:20.04', 'ubuntu:22.04'],
};
...
local any_branch = '**';
local platforms_custom = ['centos:7', 'rockylinux:8', 'debian:11', 'debian:bookworm', 'ubuntu:20.04', 'ubuntu:22.04'];
...
local debian12_deps = 'echo -e "VERSION_ID=testing\nVERSION_CODENAME=bookworm " >> /etc/os-release; apt update && apt install -y lsb-release g++';
...
local platformMap(platform, arch) =
  ...
  local platform_map = {
    ...
    'debian:bookworm': deb_build_deps + ' && ' + debian12_deps + " && sleep $${BUILD_DELAY_SECONDS:-1s} && CMAKEFLAGS='" + cmakeflags + " -DDEB=buster' debian/autobake-deb.sh",
...
local testRun(platform) =
  local platform_map = {
    ...
    'debian:bookworm': 'cd builddir; ctest -R columnstore: -j $(nproc) --output-on-failure',
...
local testPreparation(platform) =
  local platform_map = {
    ...
    'debian:bookworm': 'apt update && apt install --yes git libboost-all-dev libcppunit-dev libsnappy-dev cmake',
...
local Pipeline(branch, platform, event, arch='amd64', server='10.9') = {
  ...
  local img = if (platform == 'centos:7' || std.split(platform, ':')[0] == 'rockylinux' || platform == 'debian:bookworm')  then platform else 'romcheck/' + std.strReplace(platform, '/', '-'),
