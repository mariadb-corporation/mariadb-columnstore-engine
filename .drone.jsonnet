...
local platformMap(platform) =
  local platform_map = {
    ...
    'rockylinux:8': centos8_build_deps + ' && yum ' + rpm_build_deps + ' && cmake ' + cmakeflags + ' -DRPM=rocky8 && make -j$(nproc) package',
...
local testPreparation(platform) =
  local platform_map = {
    ...
    'rockylinux:8': ' yum install -y dnf-plugins-core libarchive && yum config-manager --set-enabled powertools && yum install -y lz4 gtest-devel cppunit-devel cmake3 boost-devel snappy-devel',
