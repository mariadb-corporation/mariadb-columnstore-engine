...
local platforms = {
  ...
  develop: ['centos:7', 'rockylinux:8', 'debian:10', 'debian:11', 'ubuntu:20.04'],
...
local deb_build_deps = 'apt update --yes && apt install --yes --no-install-recommends build-essential devscripts git ccache equivs eatmydata libssl-dev libgtest-dev && mk-build-deps debian/control -t "apt-get -y -o Debug::pkgProblemResolver=yes --no-install-recommends" -r -i ';
...
local testPreparation(platform) =
  local platform_map = {
    'centos:7': 'yum -y install epel-release && yum install -y git cppunit-devel cmake3 boost-devel snappy-devel gtest-devel',
    'rockylinux:8': rockylinux8_build_deps + ' && dnf install -y git lz4 cppunit-devel cmake3 boost-devel snappy-devel gtest-devel',
...
local Pipeline(branch, platform, event, arch='amd64', server='10.9') = {
  ...
  publish(step_prefix='pkg', eventp=event + '/${DRONE_BUILD_NUMBER}'):: {
    ...
    settings: {
      ...
      source: 'result',
      target: branchp + '/' + eventp + '/' + server + '/' + arch + '/' + std.strReplace(std.strReplace(platform, ':', ''), '/', '-'),
      delete: 'true',
    },
  },
  ...
  steps: [
           ...
           },
           {
             ...
             commands: [
               ...
               "sed -i '/embedded/d' debian/mariadb-test.install",
