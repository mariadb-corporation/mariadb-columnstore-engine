...
local platforms = {
  ...
  "develop-23.02": ["rockylinux:8", "rockylinux:9", "debian:12", "ubuntu:20.04", "ubuntu:22.04", "ubuntu:24.04"],
};
...
local Pipeline(branch, platform, event, arch="amd64", server="10.6-enterprise", customBootstrapParams="", customBuildEnvCommandsMapKey="") = {
  ...
  local branch_ref = if (branch == any_branch) then "develop-23.02" else branch,
             ...
             commands: [
                       ...
                       [
                        'bash -c "set -o pipefail && bash /mdb/' + builddir + "/storage/columnstore/columnstore/build/bootstrap_mcs.sh " +
                         ...
                         "--distro " + platform + " " +
                         "--build-packages --install-deps --sccache -g " +
