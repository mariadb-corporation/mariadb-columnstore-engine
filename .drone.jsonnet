...
local platformMap(platform) =
  local platform_map = {
    ...
    'rockylinux:8': centos8_build_deps + ' && yum ' + rpm_build_deps + ' && export CXX=/usr/bin/clang++ && export CC=/usr/bin/clang && cmake ' + cmakeflags + ' -DRPM=centos8 && make -j$(nproc) package',
