...
local Pipeline(branch, platform, event) = {
  ...
  smoke:: {
    ...
    commands: [
      ...
      if (std.split(platform, ':')[0] == 'debian') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "source /etc/lsb-release && echo \'deb http://mirror.one.com/mariadb/repo/10.5/debian $DISTRIB_CODENAME main\' > /etc/apt/sources.list.d/mariadb.list && cat /etc/apt/sources.list.d/mariadb.list"' else '',
      if (std.split(platform, ':')[0] == 'ubuntu') then 'docker exec -t smoke$${DRONE_BUILD_NUMBER} bash -c "source /etc/lsb-release && echo \"deb http://mirror.one.com/mariadb/repo/10.5/ubuntu $DISTRIB_CODENAME main\" > /etc/apt/sources.list.d/mariadb.list && cat /etc/apt/sources.list.d/mariadb.list"' else '',
