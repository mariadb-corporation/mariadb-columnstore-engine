...
local debian10_deps = 'apt update && apt install -y gnupg wget && echo "deb http://apt.llvm.org/buster/ llvm-toolchain-buster-14 main" >>  /etc/apt/sources.list  && wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - && apt update && apt install -y clang-14 libstdc++-12-dev &&' + clang14_update_alternatives;
local opensuse_build_deps = 'zypper install -y clang12 liblz4-devel cmake libboost_system-devel pcre2-devel libboost_filesystem-devel libboost_thread-devel libboost_regex-devel libboost_date_time-devel libboost_chrono-devel libboost_atomic-devel gcc-fortran gcc10 gcc10-c++ && ' + clang14_update_alternatives;
...
local platformMap(platform, arch) =
  ...
  local platform_map = {
    ...
    'debian:10': deb_build_deps + " && " + debian10_deps + turnon_clang + " CMAKEFLAGS='" + cmakeflags + " -DDEB=buster' debian/autobake-deb.sh",
...
local Pipeline(branch, platform, event, arch='amd64') = {
  ...
  local regression_tests = if (branch == 'develop') then 'test000.sh' else 'test000.sh,test001.sh',
