...
{
 public:
  Dumper(const compress::CompressInterface* comp, joblist::MemManager* mm)
   : fCompressor(comp), fMM(mm->clone())
  {
  }
...
class RowGroupStorage
{
 ...
 public:
   ...
   , fCompressor(compressor)
  {
    if (rm)
    {
      fMM.reset(new joblist::RMMemManager(rm, sessLimit, wait, strict));
      if (!wait && !strict)
      {
    ...
    {
      fMM.reset(new joblist::MemManager());
      fLRU = std::unique_ptr<LRUIface>(new LRUIface());
    }
...
class RowPosHashStorage
{
 public:
  ...
  {
    if (rm)
      fMM.reset(new joblist::RMMemManager(rm, sessLimit, !enableDiskAgg, !enableDiskAgg));
    else
      fMM.reset(new joblist::MemManager());
 ...
 , fRandDistr(0, 100)
{
  ...
  if (rm)
  {
    fMM.reset(new joblist::RMMemManager(rm, sessLimit, !enabledDiskAgg, !enabledDiskAgg));
