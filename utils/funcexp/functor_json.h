...
class Func_json
{
  ...
  public:
    ...
    initJsonArray(NULL, &jsEg.stack, sizeof(int), &jsEg_stack[0], MY_INIT_BUFFER_USED | MY_BUFFER_NO_RESIZE);
#endif
    }

    void reset_json_stack()
    {
      memset(jsEg.stack.buffer, 0, sizeof(int) * JSON_DEPTH_LIMIT/2);
    }
};
...
public:
  ...
  void init_json_path_array(bool &inited, JSONPath &path)
  {
    if (!inited)
    {
#if MYSQL_VERSION_ID >= 120100
      initJsonArray(NULL, &path.p.steps, sizeof(json_path_step_t), &p_steps[0], MY_INIT_BUFFER_USED | MY_BUFFER_NO_RESIZE);
#endif
    }
    inited= true;
  }
...
class Func_json_multipath : public Func_json
{
  ...
  Func_json_multipath()
  {
    ...
    for (size_t i=0; i<paths.size(); i++)
    {
      ...
      initJsonArray(NULL, &curr_path.p.steps, sizeof(json_path_step_t), &p_steps_arr[i][0], MY_INIT_BUFFER_USED | MY_BUFFER_NO_RESIZE);
    }
#endif
  }
  ...
  void init_json_multipath_array(bool &inited, vector<JSONPath>& paths, std::vector<std::vector<json_path_step_t>>& p_steps_arr)
    {
      if (!inited)
      {
#if MYSQL_VERSION_ID >= 120100
...
class Func_json_extract : public Func_Str, public Func_json_multipath
{
 ...
 public:
  ...
  {
#if MYSQL_VERSION_ID >= 120100
    initJsonArray(NULL, &savJSEg.stack, sizeof(int), &savJSEg_stack, MY_INIT_BUFFER_USED | MY_BUFFER_NO_RESIZE);
    initJsonArray(NULL, &p.steps, sizeof(json_path_step_t), &p_steps, MY_INIT_BUFFER_USED | MY_BUFFER_NO_RESIZE);
#endif
  }
