                                   ...
                                   CalpontSystemCatalog::ColType& /*ct*/)
{
  ...
  switch (parm[0]->data()->resultType().colDataType)
  {
    ...
    case execplan::CalpontSystemCatalog::UDOUBLE:
      return datatypes::secToTimeWithUsec(parm[0]->data()->getDoubleVal(row, isNull));

    case execplan::CalpontSystemCatalog::FLOAT:
    case execplan::CalpontSystemCatalog::UFLOAT:
      return datatypes::secToTimeWithUsec((double)parm[0]->data()->getFloatVal(row, isNull));

    case execplan::CalpontSystemCatalog::LONGDOUBLE:
      return datatypes::secToTimeWithUsec((double)parm[0]->data()->getLongDoubleVal(row, isNull));

    case execplan::CalpontSystemCatalog::DECIMAL:
    case execplan::CalpontSystemCatalog::UDECIMAL:
    {
      IDB_Decimal dec = parm[0]->data()->getDecimalVal(row, isNull);
      return datatypes::secToTimeFromDecimal(dec, curCt.precision, curCt.scale);
    }
  ...
  if (val > datatypes::TIME_MAX_WHOLE_SECONDS)
    return ("838:59:59");
                                    ...
                                    CalpontSystemCatalog::ColType& op_ct)
{
  ...
  if (val > datatypes::TIME_MAX_WHOLE_SECONDS)
    val = 8385959;
  else if (val < -datatypes::TIME_MAX_WHOLE_SECONDS)
    val = 4286581337LL;
  else
  {
