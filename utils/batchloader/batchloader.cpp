...
//------------------------------------------------------------------------------
BatchLoader::BatchLoader(uint32_t tableOid, execplan::CalpontSystemCatalog::SCN sessionId,
...
/*Look up extent map to decide which dbroot to start
        ...
        2. If the abbreviated extent has hwm > 0, and other PMs don't have any extent, start from next PM on
   the list. 2.5If some DBRoots have extents and some don't the DBRoot with the fewest extents or blocks is
   chosen, "unless" the partition 0, segment 0 extent is one of the HWM extents.  In that case the partition
   0, segment 0 extent still takes precedence.
        3. If all PMs have extents, count the number of extents under each dbroot, find the dbroot which has
   the least extents to start
...
void BatchLoader::selectFirstPM(uint32_t& PMId)
{
  ...
  uint64_t* rootExtents =
      (uint64_t*)alloca((numDbroot + 1) * sizeof(uint64_t));  // array of number extents for each dbroot
  ...
  uint64_t* rootBlocks = (uint64_t*)alloca(
      (numDbroot + 1) *
      sizeof(uint64_t));  // array of number of blocks for the last partition for each dbroot
  ...
  if (!startDBRootSet)
  {
    ...
    for (unsigned j = 0; j < fPmDistSeq.size(); j++)
    {
      ...
      for (unsigned k = 0; k < dbroots.size(); k++)
      {
        ...
        // aRootInfo.DBRoot<<":"<<aRootInfo.numExtents<<":"<<aRootInfo.numBlocks<<endl;
    ...
    // cout << "rootsExtentsBlocks size is " << rootsExtentsBlocks.size() << " and allOtherDbrootEmpty is " <<
    // allOtherDbrootEmpty<< endl;
...
void BatchLoader::buildBatchDistSeqVector(uint32_t StartPm)
{
  ...
  else
  {
    ...
    for (uint32_t i = 0; i < fPMs.size(); i++)
    {
      if ((fPMs[i] > StartPm) && (fPMs[i] <= aLast))
        aPms.push_back(fPMs[i]);
    }

    // Add all the PMs with index less than "StartPm"
    for (uint32_t i = 0; i < fPMs.size(); i++)
    {
      if ((fPMs[i] < StartPm) && (fPMs[i] >= aFirst))
        aPms.push_back(fPMs[i]);
    }
  }
