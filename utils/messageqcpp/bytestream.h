...
class ByteStream : public Serializeable
{
 public:
   ...
   */
  inline ByteStream(const uint8_t* bp, uint32_t len);
  /**
   *	copy ctor
   */
   ...
   */
  inline void advance(uint32_t adv);
...
inline ByteStream::ByteStream(const uint8_t* bp, const uint32_t len) : fBuf(nullptr), fMaxLen(0)
{
  load(bp, len);
}
