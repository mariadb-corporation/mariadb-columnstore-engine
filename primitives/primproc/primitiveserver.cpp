...
void DictScanJob::write(const SBS& sbs)
{
  ...
  std::cout << " DictScanJob::write ";
  if (!fIos)
  {
    std::cout << " send to the local queue " << std::endl;
...
int DictScanJob::operator()()
{
  ...
  SBS results(new ByteStream(1024));
  ...
  try
  {
    ...
    *fByteStream >> verInfo;
    ...
    for (uint16_t i = 0; i < runCount; ++i)
    {
      ...
      std::cout << "DictScanJob" << std::endl;
      write(results);
      results.reset(new ByteStream(1024));
      // results->restart();
      cmd->LBID++;
    }
