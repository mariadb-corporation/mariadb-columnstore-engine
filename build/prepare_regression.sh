...
if [[ -z "${CONTAINER_NAME}" ]]; then
    echo "Please provide mtr container name as a parameter, e.g. ./prepare_regression.sh -c regression183"
    exit 1
fi
...
docker cp "/mdb/${builddir}/storage/columnstore/columnstore/storage-manager" "${CONTAINER_NAME}:/"
