...
#Should be passed in order of definition!
...
install_deps()
{
    message_split
      ...
      rpm-build lsof iproute pam-devel perl-DBI cracklib-devel expect createrepo python3 checkpolicy cppunit-devel cmake3"
...
build_package() {
    modify_packaging
    #testPreparation(OS) #TODO: migrate from jsonnet:800? all deps from testPreparation are already installed in install_deps
...
if [[ $INSTALL_DEPS = true || $BUILD_PACKAGES = true ]] ; then #TODO: or it is better to avoid this implicit behavior(remove || $BUILD_PACKAGES = true ) and explicitly pass install_deps from ci?
    install_deps
fi
