...
execInnerDockerNoTTY "$CONTAINER_NAME" "export PRESERVE_LOGS=true && cd /mariadb-columnstore-regression-test/mysql/queries/nightly/alltest && bash -c \"timeout -k 1m -s SIGKILL --preserve-status ${REGRESSION_TIMEOUT} \
     ./go.sh --sm_unit_test_dir=/storage-manager --tests=${TEST_NAME} || ./regression_logs.sh ${TEST_NAME}"