...
def get_cej_info(config_root: etree._Element) -> Tuple[str, str, str, str]:
    ...
    :raises CEJError: if credentials are missing or password cannot be
                      decrypted
    """
    ...
    if not cej_username:
        raise CEJError(
            'CrossEngineSupport User is not configured in Columnstore.xml.'
        )
    if not cej_password:
        logging.warning(
    ...
    if (
        ...
        raise CEJError(
            'CrossEngineSupport password appears to be encrypted '
            'but no .secrets file exists.'
        )
...
def validate_cej_credentials(config: str = DEFAULT_MCS_CONF_PATH) -> None:
    """Validate that Cross-Engine credentials are configured and functional.

    Uses get_cej_info (which leverages CEJPasswordHandler for decryption)
    to verify that CrossEngineSupport credentials are present and valid.
    Raises CMAPIBasicError if the credentials are missing or corrupt.

    :param config: columnstore xml config file path,
                   defaults to DEFAULT_MCS_CONF_PATH
    :type config: str
    :raises CMAPIBasicError: if credentials are missing or corrupt
    """
    nc = NodeConfig()
    root = nc.get_current_config_root(config)
    ...
    try:
        ...
        raise CMAPIBasicError(
            'ColumnStore failed to start due to invalid or missing '
            'Cross-Engine credentials. '
...
def system_ready(config_filename=DEFAULT_MCS_CONF_PATH):
    ...
    try:
        ...
        logging.error("system_ready(): %s", exc)
        return False, False
    ...
    ret = subprocess.run(cmd, stdout=subprocess.PIPE, shell=True, check=False)
    if ret.returncode == 0:
