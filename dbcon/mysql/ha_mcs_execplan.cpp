...
Item* directRefOrRefLoop(const Item* item)
{
...
bool sortItemIsInGroupRec(Item* sort_item, Item* group_item)
{
  ...
  {
    found = sortItemIsInGroupRec(directRefOrRefLoop(sort_item), group_item);
  }
  ...
  for (uint32_t i = 0; !found && i < ifp_sort->argument_count(); i++)
  {
    ...
    {
      found = sortItemIsInGroupRec(directRefOrRefLoop(ifp_sort_arg), group_item);
    }
  }
...
void check_sum_func_item(const Item* item, void* arg)
{
  ...
  {
    Item* ref_item_item = directRefOrRefLoop(const_cast<Item*>(item));
...
ReturnedColumn* buildFunctionColumn(Item_func* ifp, gp_walk_info& gwi, bool& nonSupport, bool selectBetweenIn)
{
  ...
  else if ((functor = funcExp->getFunctor(funcName)))
  {
    ...
    if (funcName == "in" || funcName == " IN " || funcName == "between")
    {
      ...
      Item* ifpArg0 = directRefOrRefLoop(ifp->arguments()[0]);
...
void gp_walk(const Item* item, void* arg)
{
  ...
  switch (itype)
  {
    ...
    case Item::REF_ITEM:
    {
      Item* col = directRefOrRefLoop(item);
      ...
      else if (col->type() == Item::FUNC_ITEM)
      {
        ...
        for (uint32_t i = 0; i < ifp->argument_count(); i++)
        {
          ...
          operand =
              buildReturnedColumn(directRefOrRefLoop(ifp->arguments()[i]), *gwip, gwip->fatalParseError);
                  ...
                  bool isSelectHandlerTop, const std::vector<COND*>& condStack)
{
  ...
  if (!unionSel)
  {
    ...
    for (; groupcol; groupcol = groupcol->next)
    {
      ...
      Item* groupItem = directRefOrRefLoop(*(groupcol->item));
...

    {
      for (; ordercol; ordercol = ordercol->next)
      {
        ...
        else
        {
          ...
          if (!rc || gwi.fatalParseError)
          {
            Item* item_ptr = directRefOrRefLoop(ordercol->item_ptr);
  ...
  // ORDER BY processing starts here
  {
    ...
    for (; ordercol; ordercol = ordercol->next)
    {
      ...
      else
      {
        ...
        {
          gwi.fatalParseError = false;
          rc = buildReturnedColumn(directRefOrRefLoop(ordercol->item_ptr), gwi, gwi.fatalParseError);
        }
