...
#define STF(x) " " << #x << ": " << ((x) ? "true" : "false")
  idblog(STF(isUnion) << STF(isSelectHandlerTop) << STF(isSelectLexUnit) << STF(select_lex.master_unit()->is_unit_op()));
  //if (!isUnion && (!isSelectHandlerTop || isSelectLexUnit) && select_lex.master_unit()->is_unit_op())
  if (!isUnion && (!isSelectHandlerTop ) && select_lex.master_unit()->is_unit_op())
  {
    // MCOL-2178 isUnion member only assigned, never used
...
// XXX: need to get rid of that altogether.
