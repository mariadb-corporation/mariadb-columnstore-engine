...
class JobStepAssociation
{
 public:
  ...
  {
    if (pos > fOutDataList.size())
      throw std::logic_error("Insert position is beyond end.");
...
class JobStep
{
 public:
  ...
  void syslogStartStep(uint32_t subSystem, const std::string& stepName) const;
  void syslogEndStep(uint32_t subSystem, uint64_t blockedDLInput, uint64_t blockedDLOutput,
                     uint64_t msgBytesInput = 0, uint64_t msgBytesOutput = 0) const;
  void syslogReadBlockCounts(uint32_t subSystem, uint64_t physicalReadCount, uint64_t cacheReadCount,
                             uint64_t casualPartBlocks) const;
  void syslogProcessingTimes(uint32_t subSystem, const struct timeval& firstReadTime,
  ...
  void handleException(std::exception_ptr e, const int errorCode, const unsigned infoErrorCode,
