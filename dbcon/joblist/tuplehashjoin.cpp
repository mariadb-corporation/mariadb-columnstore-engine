...
void TupleHashJoinStep::trackMem(uint index)
{
  ...
  while (!stopMemTracking)
  {
    ...
    memTrackDone.wait_for(scoped, std::chrono::duration::seconds(1));
  }
...
void TupleHashJoinStep::startSmallRunners(uint index)
{
  ...
  */
  ...
  std::scoped_lock lk(*fStatsMutexPtr);
...
uint32_t TupleHashJoinStep::nextBand(messageqcpp::ByteStream& bs)
{
  ...
  std::scoped_lock lk(deliverMutex);
...
{
  std::scoped_lock lock(outputDLLock);
...
{
  std::scoped_lock lock(inputDLLock);
  work->clear();
