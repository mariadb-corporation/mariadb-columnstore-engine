...
ConstantFilter::ConstantFilter(const ConstantFilter& rhs) : Filter(rhs), fOp(rhs.fOp), fCol(rhs.fCol)
{
  ...
  for (uint32_t i = 0; i < rhs.fFilterList.size(); i++)
  {
    ...
    fFilterList.push_back(ssfp);
    fSimpleColumnList.insert(fSimpleColumnList.end(), ssfp->simpleColumnList().begin(),
                             ssfp->simpleColumnList().end());
    fAggColumnList.insert(fAggColumnList.end(), ssfp->aggColumnList().begin(), ssfp->aggColumnList().end());
...
bool ConstantFilter::hasAggregate()
{
  if (fAggColumnList.empty())
  {
    for (uint32_t i = 0; i < fFilterList.size(); i++)
    {
      ...
      {
        fAggColumnList.insert(fAggColumnList.end(), fFilterList[i]->aggColumnList().begin(),
...
bool ConstantFilter::operator==(const ConstantFilter& t) const
{
  ...
  for (it = fFilterList.begin(), it2 = t.fFilterList.begin(); it != fFilterList.end(); ++it, ++it2)
  {
    if (**it != **it2)
      return false;
  }
...
void ConstantFilter::correctSQLText()
{
  ...
  for (unsigned i = 0; i < fFilterList.size(); i++)
  {
    ...
    // debian 12 has problems with code like "delimiter = ",";", thus this.
    if (i == 0)
    {
      values << "(";
    }
  ...
  data ("`" + sc->schemaName() + "`.`" + sc->tableAlias() + "`.`" + sc->columnName() + "` " +
...
void ConstantFilter::setSimpleColumnList()
{
  ...
  for (uint32_t i = 0; i < fFilterList.size(); i++)
  {
    ...
    fSimpleColumnList.insert(fSimpleColumnList.end(), fFilterList[i]->simpleColumnList().begin(),
