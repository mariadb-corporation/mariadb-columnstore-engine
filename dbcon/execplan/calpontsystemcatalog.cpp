                                                          ...
                                                          int lower_case_table_names)
{
  ...
  std::scoped_lock lk2(fOIDmapLock);
  ...
  std::scoped_lock lk3(fColinfomapLock);
...
const CalpontSystemCatalog::IndexNameList CalpontSystemCatalog::colValueSysconstraint (const TableColName& tableColName, bool useCache)
{
    ...
    std::scoped_lock lk1(fColIndexListmapLock, false);
#else
    std::scoped_lock lk1(fColIndexListmapLock, boost::defer_lock);
#endif
                                                                     ...
                                                                     int lower_case_table_names)
{
  ...
  std::scoped_lock lk1(fTableInfoMapLock);
...
#if BOOST_VERSION < 103800
  std::scoped_lock lk2(fOIDmapLock, false);
#else
  std::scoped_lock lk2(fOIDmapLock, boost::defer_lock);
#endif
  std::scoped_lock lk3(fColinfomapLock);
...
const CalpontSystemCatalog::TableName CalpontSystemCatalog::tableName(const OID& tableoid)
{
  ...
  // check cache
  std::scoped_lock lk(fTableNameMapLock);
                                                                  ...
                                                                  int lower_case_table_names)
{
  ...
  std::scoped_lock lk1(fTableAUXColumnOIDMapLock);
...
CalpontSystemCatalog::OID CalpontSystemCatalog::isAUXColumnOID(const OID& oid)
{
  ...
  std::scoped_lock lk1(fAUXColumnOIDToTableOIDMapLock);
...
void CalpontSystemCatalog::getSchemaInfo(const string& in_schema, int lower_case_table_names)
{
  ...
  std::scoped_lock lk(fSchemaCacheLock);
...
{
  std::scoped_lock lk1(fOIDmapLock);
  fOIDmap.clear();
  buildSysOIDmap();
  lk1.unlock();

  std::scoped_lock lk2(fColinfomapLock);
  fColinfomap.clear();
  buildSysColinfomap();
  lk2.unlock();

  std::scoped_lock lk3(fTableInfoMapLock);
  fTableInfoMap.clear();
  fTablemap.clear();
  fTableRIDmap.clear();
  buildSysTablemap();
  ...
  std::scoped_lock namemaplk(fTableNameMapLock);
  fTableNameMap.clear();
  namemaplk.unlock();

  std::scoped_lock auxlk(fTableAUXColumnOIDMapLock);
  fTableAUXColumnOIDMap.clear();
  auxlk.unlock();

  std::scoped_lock auxtotableoidlk(fAUXColumnOIDToTableOIDMapLock);
...
{
  std::scoped_lock lk(fColinfomapLock);
...
void CalpontSystemCatalog::checkSysCatVer()
{
  ...
  std::scoped_lock sysCatLk(fSyscatSCNLock);
